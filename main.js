...
.then(function(response) {
    ...
    console.log(response);
        ...
        for(var i = 0; i < cityArray.length; i++ ){
            ...
            citybutton.attr("data-name", cityArray[i]);
            var btnDiv = $("<div>")
            citybutton.text(cityArray[i]);
            btnDiv.append(citybutton)
            $("#btnStorage").append(btnDiv);
            // create a div to store button 
            //set the class for the button
            //grab the value of the current index of the array
            //and store it in the button
            //then append the button into the div i made
            //and then append the div to the page 
